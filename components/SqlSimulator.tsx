    ...
    <div className="space-y-6">
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            {useRealApi && (
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-all text-sm font-medium whitespace-nowrap"
              >
        ...
        {useRealApi && connectionStatus === 'success' && (
            <div className="mb-6 bg-indigo-900/20 border border-indigo-500/50 rounded-lg p-4 flex flex-col md:flex-row md:items-center gap-4 animate-fade-in">
                <div className="p-3 bg-indigo-500/20 rounded-full text-indigo-400 self-start md:self-center">
                    <Server className="w-6 h-6" />
                </div>
                <div className="flex-1">
                    <h4 className="text-indigo-300 font-bold text-lg flex items-center gap-2">
                        Connected to: <span className="text-white underline underline-offset-4">{connectedDbName}</span>
                    </h4>
                    <p className="text-indigo-400/70 text-xs font-mono mt-1">{connectionMsg}</p>
                </div>
                <div className="bg-slate-950 px-3 py-2 rounded border border-slate-700 text-right">
                    ...
                    <div className={`text-sm font-mono ${backendVersion.includes('v5.4') ? 'text-indigo-400' : 'text-red-400'}`}>
                        {backendVersion}
                    </div>
                </div>
            </div>
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg py-4 pl-12 pr-24 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
          />
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 w-5 h-5" />
          <button
            onClick={handleSimulate}
            disabled={loading || !input.trim()}
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-md font-medium text-sm transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {result && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in">
          <div className="bg-slate-900 rounded-xl border border-slate-700 overflow-hidden flex flex-col">
            ...
            <div className="p-4 font-mono text-sm text-indigo-300 overflow-auto flex-1 whitespace-pre-wrap">
              {result.sql}
            </div>
          </div>
